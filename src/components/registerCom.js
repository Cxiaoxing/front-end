...
import { SendCodeRequest, QuickLoginRequest } from '../api/account'
...
class RegisterCom extends React.Component {
        ...
        return (
            <div className="form-wrap">
                <div className="form-header">
                    <span className="form-title">快速登陆</span>
                    <span className="to_register" onClick={() => { this.props.onChange() }}>用户密码登陆</span>
                </div>
                <form className="form-content">
                    <List className="login-form">
                        ...
                        >
                            <InputItem
                                type="phone" placeholder="Mobile"
                                {...getFieldProps('mobile', {
                                    rules: [
                                        { required: true, message: '请输入手机号码！' },
                                        { pattern: /^1[0-9]{2} [0-9]{4} [0-9]{4}$/, message: '请输入正确的手机号码！' },
                                    ],
                                })}
                        ...
                        >
                            <div className="grid">
                                <InputItem
                                    ...
                                    onErrorClick={() => {
                                        Toast.fail(getFieldError('verification_code').join(' '));
                                    }}
                                />
                                <span>&nbsp;&nbsp;</span>
                        ...
                        <List.Item className="form-item">
                            <Button type="primary" size="large" onClick={this.onFinish} className="login-form-button">登陆或注册</Button>
                        </List.Item>
                    </List>
                </form>
            </div>
    ...
    encodeUri = (data) => {
        return encodeURIComponent(JSON.stringify(data)).replace(/%7B%22/g, "").replace(/%22%3A%22/g, "=").replace(/%22%2C%22/g, "&").replace(/%22%7D/g, "").replace(/%20/g, "")
    }
    ...
    onSend = () => {
        this.props.form.validateFields({ force: true }, (error) => {
            if (!error) {
                // 调用发送验证码接口
                SendCodeRequest(this.encodeUri(this.props.form.getFieldsValue())).then(response => {
    ...
    onFinish = () => {
        this.props.form.validateFields({ force: true }, (error) => {
            if (!error) {
                if (this.props.form.getFieldsValue().verification_code) {
                    // 调用快速登陆接口
                    QuickLoginRequest(this.encodeUri(this.props.form.getFieldsValue())).then(response => {
                        Toast.success('快速登录成功，即将跳转到首页！')
                        console.log(response)
                        setTimeout(()=>{window.location.replace("/")},1000)
                    }).catch(Error => {
                        console.log(Error)
                        Toast.fail('快速登陆失败，请检查手机号或验证码是否正确！');
                    })
                } else {
                    Toast.fail('请输入验证码！');
                }
            } else {
                Toast.fail('快速登陆失败，请注意提示信息！');
            }
        });
    }
}
